---
title: "Imports are not values"
description: "Jotting down some of my learnings about live import bindings and how they (don't) work in default exports."
createdAt: 07-27-2025
image: "../assets/imports-are-not-values.png"
draft: false
tags:
  - javascript
---
Imports in JavaScript are not static values, but references that are dynamic.

They are called live bindings.

### Understanding live bindings

Suppose you have a module which exports a variable like this:

```javascript
// foo.js
let foo = 'initialValue';

export { foo };
```

You import it like this in another module:

```javascript
// index.js
import { foo } from './foo.js';

console.log(foo);
```

This will print `"initialValue"` as expected.

Now, if we change `foo` after exporting it:

```javascript
// foo.js
let foo = 'initialValue';

export { foo };

foo = 'changedValue';
```

This prints `"changedValue"`.

This means the imported `foo` is not just a static copy, but its a reference to the original variable in the exporting module

It seems obvious, and well, it kind of is. But things get interesting with default exports.

### Default exports

When we switch the same code to use default exports:
```javascript
// foo.js
let foo = 'initialValue';

export default foo;

foo = 'changedValue';
```

```javascript
// index.js
import foo from './foo.js';

console.log(foo);
```

It prints `"initialValue"` even though the original variable was modified to `"changedValue"`. Why did it happen?

Well, default exports treat the exported value as an expression, not a live binding. 

This is a feature of the default exports spec, and not a bug, because it allows us to do things like:
```javascript
export default 'hello!';
```
Or this:
```javascript
export default 1 + 2;
```

But it also means that we lose the live connection to the original variable.

Things get weird with function and class declarations though.

### Weird case

If we do:
```javascript
// foo.js
export default function foo () {
    console.log('hello, world!');
}

foo = 'changedValue';
```

In this case the default import prints `"changedValue"`, how?

Turns out function and class declarations are an exception to the default exports' "exported as expression" rule. And they're treated as live bindings again.

This does not really make sense to me.

If the browser is able to detect if the exported value is a function/class declaration and treat it as a live binding, then exported variables like `export default variable` should've also been a binding. 

But it doesn't work this way.

There is a workaround though, to make default exports live.

### Workaround

By using named export syntax:
```javascript
// foo.js
let foo = 'initialValue';

export { foo as default };

foo = 'changedValue';
```

This works as expected and prints `"changedValue"`

In my opinion this is not really intuitive. Default exports don't always behave like normal exports.

If someone's unaware of the differences, it can create unintentional bugs in large codebases.

And that's why I am going to stop using default exports as much as possible, and I think you should too!
